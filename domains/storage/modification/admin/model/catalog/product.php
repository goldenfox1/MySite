		...
		$product_id = $this->db->getLastId();
			...
			///xml///
				...
				if (isset($data['product_link'])) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_link SET product_id = '" . (int)$product_id . "', link = '" . $this->db->escape($data['product_link']) . "'");
				}
	...
	public function editProduct($product_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
			...
			///xml///
				...
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_link WHERE product_id = '" . (int)$product_id . "'");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data['product_recurrings'] = $this->getRecurrings($product_id);
				...
				$data['product_link'] = $this->getProductLink($product_id);
	...
	public function deleteProduct($product_id) {
		...
		$this->cache->delete('product');
				...
				public function getProductLink($product_id) {
					$query = $this->db->query("SELECT link FROM `" . DB_PREFIX . "product_link` WHERE product_id = '" . (int)$product_id . "'");
					if (empty($query->row['link'])) {
						$query->rows;
					} else {
